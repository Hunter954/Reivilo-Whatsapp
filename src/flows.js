...
async function handleIncomingMessage(client, message) {
  ...
  if (n === '2' || n.includes('quero fazer parte') || n.includes('pagamento')) {
    ...
    await send(client, jid, checkoutIntro(), user.id);
    try {
  ...
  if (n === '3' || n.includes('atendente') || user.onboarding_step === 'support') {
    ...
    if (n === '3' || n.includes('atendente')) return send(client, jid, supportMessage(), user.id);
